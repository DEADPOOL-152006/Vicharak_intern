...
2. Upload `bitstream/spi_loopback_led.bin` using ShrikeFlash or GCSH
3. Flash the RP2040 with CircuitPython
4. Upload `firmware/arduino-ide/code.py` to the root of the microcontroller
5. Upload `firmware/arduino-ide/top512.txt` dictionary file to the root
6. Connect the SPI jumper wires between RP2040 and Shrike-Lite
7. Run the code and type a misspelled word in the terminal

## Build From Source

### FPGA (Verilog)
1. Open `spi_loopback_led.ffpga` in Go Configure Software Hub
2. Load `fpga/src/top.v` and `fpga/src/spi_target.v` into your project
3. Apply your `.cst` Physical Constraints file to map the SPI pins and 50MHz clock
4. Click Synthesize → Generate Bitstream
5. Output will be in `fpga/build/`

### Firmware (RP2040 / CircuitPython)
1. Flash your RP2040 with **CircuitPython** (recommended for native USB HID support)
2. Open `firmware/arduino-ide/code.py` and upload to the root of your microcontroller
3. Upload `firmware/arduino-ide/top512.txt` dictionary file to the root
...
- Handles hardware synchronization with a 5µs micro-delay to allow FPGA clock alignment
...
- Uses a 3-stage synchronizer to catch the RP2040's CS drop and align the 100kHz SPI clock
...
4. **Hardware Synchronization:** A 5µs micro-delay allows the FPGA's 50MHz clock to register the CS drop before transmission begins
5. **Hardware Echo:** FPGA registers bytes and streams the result back over MISO
6. **Output:** RP2040 displays the echoed result in the terminal

## Expected Output

When you run `code.py`:
